...
if (not infile) {cout << "Error opening file" << endl; return 1; }
...
int readDatafromfile(string filename, int students[], int tests[][numtests])
    int numstudents = 0;
    while (count < maxstudents && infile >> names[count]) {for (int i = 0; i < numtests; i++) {infile >> tests[count][i];} count++; }
}
//calculateaverages for each student
void calculateaverages(int tests[][numtests], double averages[], int numstudents)
    for (int i = 0; i < numstudents; i++) {double sum = 0; for (int j = 0; j < numtests; j++) {sum += tests[i][j];} averages[i] =
//getlettergrade function
char getlettergrade(double average)
    if (average >= 90) {return 'A';} else if (average >= 80) {return 'B';} else if (average >= 70) {return 'C';} else if (average >=60) {return 'D';} else {return 'F';}
